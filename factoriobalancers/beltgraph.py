...
class BeltGraph(Graph):
    ...
    def insert_graph(self, graph: 'BeltGraph') -> tuple[tuple, tuple]:
        '''
        Insert a graph separately into this graph.  Return `(inputs, outputs)` where `inputs` are the vertices that had corresponding inputs in `graph`, and `outputs` are the vertices that had corresponding outputs in `graph`.
        '''
    ...
    def insert_graph_between(self, graph: 'BeltGraph', from_vertices: list[int], to_vertices: list[int]) -> tuple[tuple, tuple]:
        '''
        Insert a graph between two sets of vertices.  Return `(inputs, outputs)` where `inputs` are the vertices that had corresponding inputs in `graph`, and `outputs` are the vertices that had corresponding outputs in `graph`.

        This is the same return value as `BeltGraph.insert_graph()`.
        '''
        ...
        for u, v in zip(from_vertices + list(output_vertices), list(input_vertices) + to_vertices):
            if self.can_add_edge(u, v):
                ...
                raise Exception(f'Failed to connect {self.vertex_to_str(u)} to {self.vertex_to_str(v)}.  This is likely due to vertex {self.vertex_to_str(u)} having too many outbound edges.  Try deleting edges before inserting the graph between these vertices.')
    ...
    def dfs(self, u: int = -1):
        ...
        if u == -1:
            ...
            yield from dfs(u)
    def dfs_bidirectional(self, u: int = -1):
        '''
        Depth-first search that goes in both directions, starting from the given vertex `u`.  If `u` is `-1`, then start from all input vertices and output vertices.
        '''
        visited = set()
        def dfs(u: int):
            if u in visited:
    ...
    def disjoint_vertices(self) -> list[list[int]]:
        '''
        Return a list of lists of vertices, where each list of vertices belongs to a subgraph, but each of the subgraphs are disjoint.
        '''
        # DFS to locate all vertices in the same subgraph.
        vertices = []
        for u in self.vertices():
            if any(u in v for v in vertices):
                continue
            vertices.append(list(self.dfs_bidirectional(u)))
        return vertices
...
def add_balancer(balancer_type: str, compact_string: str) -> None:
    common_balancers[balancer_type] = compact_string

def get_balancer(balancer_type: str) -> str:
    '''
    Return the compact string for the balancer type, if it's defined.
    '''
